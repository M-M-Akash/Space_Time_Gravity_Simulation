...
float gridRaise = 1000.0f; // raise the warped grid toward the planets — increase to lift grid up
...
class Object {
    public:
        ...
        float radius;
        float displayScale = 1.0f; // visual-only scale, does not affect collision

        glm::vec3 LastPos = position;
        bool glow;

        Object(glm::vec3 initPosition, glm::vec3 initVelocity, float mass, float density = 3344, glm::vec4 color = glm::vec4(1.0f, 0.0f, 0.0f, 1.0f), bool Glow = false, float displayScale = 1.0f) {   
        ...
        std::vector<float> Draw() {
            ...
            for(float i = 0.0f; i <= stacks; ++i){
                ...
                for (float j = 0.0f; j < sectors; ++j){
                    ...
                    float r = this->radius * this->displayScale;
...
std::vector<float> UpdateGridVertices(const std::vector<float>& flatVertices, const std::vector<Object>& objs);
...
int main() {
    ...
    //    Object(glm::vec3(5000, 650, -350), glm::vec3(0, 0, -1500), 5.97219*pow(10, 22), 5515, glm::vec4(0.0f, 1.0f, 1.0f, 1.0f)),
       ...
       // Mercury: closest to the star, circular orbital velocity ~931 for r=1500
       Object(glm::vec3(1500, 0, -350), glm::vec3(0, 0, 931), 1.301e23f, 5427, glm::vec4(0.7f, 0.7f, 0.7f, 1.0f)),
    //    Object(glm::vec3(1500, 0, -350), glm::vec3(0, 0, 931), 3.301e23f, 5427, glm::vec4(0.7f, 0.7f, 0.7f, 1.0f)),
       Object(glm::vec3(2800, 0, -350), glm::vec3(0, 0, 681), 2.30e23f, 5427, glm::vec4(0.95f, 0.87f, 0.70f, 1.0f)),
       // Earth: r=3876, v=579, blue-green, displayScale=3.0 makes it visually larger without affecting collision
       Object(glm::vec3(3876, 0, -350), glm::vec3(0, 0, 579), 3.0e23f, 5427, glm::vec4(0.13f, 0.37f, 0.80f, 1.0f), false, 3.0f),
    };
    std::vector<float> flatGridVertices = CreateGridVertices(20000.0f, 25, objs);
    std::vector<float> gridVertices = flatGridVertices; // working copy for rendering
    ...
    while (!glfwWindowShouldClose(window) && running == true) {
        ...
        gridVertices = UpdateGridVertices(flatGridVertices, objs);
...
std::vector<float> CreateGridVertices(float size, int divisions, const std::vector<Object>& objs) {
    ...
    float y = 0.0f; // flat grid at y=0 where planets are
    // x axis
...
std::vector<float> UpdateGridVertices(const std::vector<float>& flatVertices, const std::vector<Object>& objs){
    std::vector<float> vertices = flatVertices; // always warp from the original flat grid
    int numVerts = vertices.size() / 3;

    // Pass 1: compute displacement for each vertex and find the maximum
    std::vector<float> displacements(numVerts, 0.0f);
    float maxDisp = 0.0f;
    for (int i = 0; i < numVerts; ++i) {
        ...
        for (const auto& obj : objs) {
            ...
            float distance_m = glm::length(toObject) * 1000.0f;
            float rs = (2.0f * G * obj.mass) / (c * c);
            if (distance_m > rs)
                totalDisp += 4.0f * sqrt(rs * (distance_m - rs));
        }
        displacements[i] = totalDisp;
        if (totalDisp > maxDisp) maxDisp = totalDisp;
    }

    // Pass 2: apply inverted warp — center sinks, edges stay at y=0
    // grid_y = disp - maxDisp  (always <= 0, deepest near massive objects)
    if (maxDisp > 0.0f) {
        for (int i = 0; i < numVerts; ++i) {
            vertices[i*3+1] = displacements[i] - maxDisp + gridRaise;
        }
    }
